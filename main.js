...
function setSecret(){ //chooses a new secret word
    ...
    if(storedSecret[0] === ""){ //ensures no secret resets if the previous game is still incomplete
        ...
        console.log("The secret is still " + getSecretString()); //!uncomment this to have the secret outputted to the console
    }
}

function setSucces(){ //used on page load to set success to true if necessary
    for(let i = 0; i < 5; i+=1){//success is true if the user has guessed the word
    ...
    if(guess[5][0] !== ""){//success is true if the user has exhausted all guesses
        success = true;
        setTimeout(showStats, 1000);
    }
}
...
function createRow(row){ //creates the line of squares for the corresponding row
    if(guess[row-1][0] === ""){
        ...
        document.getElementById(elementID).innerHTML = 
            `
                <div class="square${getCorrectClass(guess[row-1][0], 1, row, true)}">
                    ${guess[row-1][0]}
                </div>
                <div class="square${getCorrectClass(guess[row-1][1], 2, row, true)}">
                    ${guess[row-1][1]}
                </div>
                <div class="square${getCorrectClass(guess[row-1][2], 3, row, true)}">
                    ${guess[row-1][2]}
                </div>
                <div class="square${getCorrectClass(guess[row-1][3], 4, row, true)}">
                    ${guess[row-1][3]}
                </div>
                <div class="square${getCorrectClass(guess[row-1][4], 5, row, true)}">
                    ${guess[row-1][4]}
                </div>
            `;
    }
...
function getCorrectClass(letter, position, row, css){ //returns the appropriate CSS class or color for letter with regards to its' position and row
    if(secret[position-1] === letter){ //correct letter, correct position
        letters[letter.toLowerCase()][1] = 2;
        if(css === true){ //css will be true when the user is entering guesses
            generateKeyboard();
            return ' greenSquare';
        }
        return '\u{1F7E9}'; //css will be false when the user wants to copy results to their clipboard
...
function insertLetter(x){ //inserts x in the next active location
    if(success === false && activeColumn < 6){ //caps guess at 5 letters
...
function showStats(){ //generates a popup showing user statistics
    ...
    <div class="popup">
        ...
        <div onclick="copyResults()" class="play-again${wordGuessed()}">
...
async function showDefinition(){ //shows the definition of the secret word
...
function hideDefinition(){ //hides the definition
    document.getElementById("def-here").innerHTML = ``;
}
...
function wordGuessed(){ //returns the CSS class to hide the copy button if the word was not guessed
    if(success === true){
        return ``;
    }
    return ` hide`;
...
function copyResults(){ //copies a series of squares to the users clipboard
    let score = userData["mostRecentScore"];
    let copyText = "";
    if(score > 6){
        copyText += "I guessed the word " + getSecretString().toLowerCase() + " in X/6 tries.\n\n";
        score = '6';
    }else{
        copyText += "I guessed the word " + getSecretString().toLowerCase() + " in " + score + "/6 tries.\n\n";
    }

    for(let row = 1; row <= score; row += 1){ //adding properly colored squares
        for(let position = 1; position <= 5; position += 1){
            copyText += getCorrectClass(guess[row-1][position-1], position, row, false);
        }
        copyText += '\n';
    }
    ...
    setTimeout(hideCopy, 1500);
}

function hideCopy(){ //hides the confirmation of a successful copy
